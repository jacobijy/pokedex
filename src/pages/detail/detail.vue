<template>
    <view class="container">
        ...
        <!-- 底部导航栏 -->
        <view
            class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-3 flex justify-around items-center">
...
onMounted((options: { id: number }) => {
    fetchPokemonDetail(options.id);
})
...
const fetchPokemonDetail = async (id: number) => {
    try {
        ...
        pokemon = {
            ...
            name: '妙蛙种子',
            types: ['草', '毒'],
            image: 'https://picsum.photos/seed/bulbasaur/200/200',
            stats: [],
            evolutionChain: [],
            moves: [],
            abilities: [],
